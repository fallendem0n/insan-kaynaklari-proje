...
# pdf okuma kütüphanelerini yüklemeye çalışıyoruz
try:
...
class PDFToTXTFrame(ctk.CTkFrame):
    def __init__(self, master=None):
        ...
        # tesseract ve poppler yollarını ayarlıyoruz
        # bu araçlar ocr işlemi için gerekli
    ...
    # dönüştürme işlemini başlatma (thread ile)
    def start_conversion_thread(self):
        """
        dönüştürme işlemini arayüzü kilitlememesi için
        ayrı bir thread'de (iş parçacığında) başlatır.
        """
    ...
    # asıl dönüştürme fonksiyonu
    def convert_to_txt(self):
        try:
            full_text = ""
            
            # 1. adım: normal metin çıkarma (pypdf ile)
            if PdfReader:
                try:
    ...
    # arayüzü sıfırlama
    def ui_reset(self):
        """arayüz bileşenlerini tekrar aktif hale getirir."""
        self.convert_button.configure(state="normal")
